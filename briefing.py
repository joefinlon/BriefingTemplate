...
def get_args():
    parser = argparse.ArgumentParser(
        description='User arguments to initialize campaign weather briefing template software.')
    parser.add_argument('--model-prod-path', type=str, required=True,
                        help='Path to the external Python file that defines model products.')
    parser.add_argument('--download-path', type=str, required=True,
                        help='Path to where graphics will be downloaded.')
    parser.add_argument('--presentation-path', type=str, required=True,
                        help='Path to where PowerPoint will be saved')
    parser.add_argument('--presentation-type', type=str, default='morning',
                        help='Either morning or evening briefing type.')
    parser.add_argument('--presentation-hour', type=int, default=13,
                        help='2-digit UTC hour when the briefing will be given.')
    parser.add_argument('--utc-offset', type=int, required=True,
                        help='Number of hours offset west of UTC dateline (positive)')
    parser.add_argument('--initialize-time', type=str, default='now',
                        help='Either now or YYYYMMDDHH to designate as the start of the Day 0 period.')
    parser.add_argument('--region', type=str, default='atne',
                        help='Region string for which region-specific sites (e.g., radar, soundings) are used.')
    parser.add_argument('--cross-section', type=str, default='nw-se',
                        help='Orientation for pre-generated model cross sections (IMPACTS only)')
    parser.add_argument('--show-weather', type=str, default='True',
                        help='whether to make slides showing the past and current weather')
    parser.add_argument('--show-shortterm', type=str, default='True',
                        help='whether to make slides showing the Day 0-2 forecast')
    parser.add_argument('--show-longterm', type=str, default='True',
                        help='whether to make slides showing the Day 3-5 forecast')
    parser.add_argument('--show-detailed', type=str, default='False',
                        help='whether to make slides showing the detailed Day 0-1 forecast')
    parser.add_argument('--briefing-update', type=str, default='False',
                        help='whether to save the presentation as a new filename')
...
def query_gmao(product, scope, dt_object, fhr):
    '''
    Initiate HTTP request so that GMAO site can create the image for saving.
    
    Parameters
    ----------
    urlpath: URL string
    '''
    dt_fullstr = datetime.strftime(dt_object, "%Y%m%dT%H%M%S")
    urlpath = (
        f'https://fluid.nccs.nasa.gov/missions/chem2d_mission+WHyMSIE//?stream=G5FPFC&field='
        f'{product}&level=0&fcst={dt_fullstr}&region={scope}&tau={str(fhr)}'
    ...
    try:
        ...
        print(f'Cannot query URL for GMAO {product}_{scope}_{dt_fullstr}')
...
def dt_nearestTimes(yr, mn, dy, hr):
    ...
    if hr<5:
        ...
        past12hr = datetime(yr, mn, dy, 0, 0)
        current12hr = past12hr
        nearest12hr = datetime(yr, mn, dy, 6, 0)
...
def dt2string(dt_object, init_time, utc_offset):
    dt_utc_full = datetime.strftime(dt_object, '%Y%m%d%H%M')
    dt_utc_short = datetime.strftime(dt_object, '%H UTC %m/%d')
    dt_local = datetime.strftime(dt_object - timedelta(hours=utc_offset), '%a %-I%p %m/%d')
...
def get_latest_image(product):
    ...
    if remotefile==localfile: # verify that a non-URL image is on the local server
        ...
        try:
            ...
            response = requests.get(remotefile, localfile, headers=headers)
            if response.status_code == 200: # save the image
...
def get_meteogram_url(nearest1hr, location='KWAL'):
    ...
    if location=='KEDW':
        meteogram_url = (
            'https://forecast.weather.gov/meteograms/Plotter.php?lat=34.9295&lon=-117.9277&'
            f'wfo=HNX&zcode=CAZ339&gset=15&gdiff=5&unit=0&tinfo=PY8&ahour={str(starthr)}'
            '&pcmd=11001110100000001000000000000000000000000000000000000100000'
            f'&lg=en&indu=0!1!1!0&dd=&bw=&hrspan={str(endhr)}&pqpfhr=6&psnwhr=3.png'
        )
...
def get_gpm_overpasses(present_time, utc_offset, zoom=None):
    '''
    ...
    for gpm_date in gpm_date_list:
        if (gpm_date > present_time) and (filenum<6):
            ...
            gpm_fullStr, gpm_shortStr, gpm_locStr, gpm_delta = dt2string(
                gpm_date, present_time, utc_offset)
...
def bumper_slide(prs, title, curday, start_time, end_time):
    ...
    # Add timeline
    dayposs = re.search(r'Day (\d)', title)
...
def full_slide_image(prs, product, curday, imgTime, ftime=None, width=None, height=None, link=False, title=None):
    ...
    if results[0] is None:
        ...
        dayposs = re.search(r'Day (\d)', product)
        add_timeline(slide, curday)
    ...
    if height is not None:
        ...
        try:
            pic = slide.shapes.add_picture(results[0], left=Inches(left_balanced), top=Inches(1.55),
        ...
        left_balanced = (10-width)/2.
        try:
            pic = slide.shapes.add_picture(results[0], left=Inches(left_balanced), top=Inches(1.55), width=Inches(width))
        except IOError:
            pic = None
    else:
        try:
...
def four_panel_image(prs, products, curday, link=False, widthval=None, heightval=None):
    images = []
    for panelnum in range(len(products)):
        results = get_latest_image(products[panelnum])
...
print('##################################################')
print('######## FORECAST PRESENTATION GENERATOR #########')
print('##################################################')
print('- This code generates a .PPT file of observation')
print('and model fields for use in the campaign weather briefings.')
...
args = get_args()
prodPath = args.model_prod_path
downloadPath = args.download_path
presentationPath = args.presentation_path
presentationType = args.presentation_type
presentationHour = args.presentation_hour
utc_offset = abs(args.utc_offset) # only supports TZ west of prime meridian for now
modelList = 'default' # currently defaults to models in region dict
region = args.region
xSection = args.cross_section
show_weather = args.show_weather
show_shortTerm = args.show_shortterm
show_longTerm = args.show_longterm
show_detailed = args.show_detailed
briefingUpdate = args.briefing_update
##################################################
### INITIALIZE THE PROGRAM BASED ON THE NAMELIST VALUES
if args.initialize_time == 'now':
    utcnow = datetime.utcnow()
else:
    utcnow = datetime.strptime(args.initialize_time, '%Y%m%d%H')
    print(
...
if presentationType=='morning':
    ...
    if utcnow.hour < 8 or utcnow.hour > 19:
        print(
...
region_dict = {
    ...
    'ussw': {
        ...
        'models':[
            'gfs', 'ecmwf', 'nam', 'wrfuiuc', 'gefs', 'cmc', 'eps',
            'hrrr', 'fv3', 'nam3km', 'rgem', 'href', 'geos', 'gem'
        ],
        'radarLoc':'LOT',
        'soundLocs':['APX','DTX','GRB','DVN'],
        'plumeLocs':['APX','DTX','GRB','DVN']
...
# import external .py file containing dict of products and their models, scopes, times
# dynamically import the config file as a module
spec = importlib.util.spec_from_file_location('model_products_config', args.model_prod_path)
config_module = importlib.util.module_from_spec(spec)
spec.loader.exec_module(config_module)

# retrieve the dictionary from the imported module
modelProducts = config_module.modelProducts
##################################################
...
[rad_edw_url, edw_datestr] = get_latest_radar('EYX', nearest1hr)

# Get URLs for the most recent meteograms
mgram_edw2_url = (
    'https://har.gsfc.nasa.gov/storm/WEATHER/ASCII_WH2yMSIE_Airfield_Conditions/'
    f'KEDW_ER-2_Weather_Analysis_{nearest1hr_fullStr[-4:-2]}UTC.png'
)
...
img_paths = {
    'rad_current_edw': (
        ...
        'radar_edw.png', 'Edwards AFB Radar', current_shortStr, current_locStr, current_delta),
    'mgram_edw2': (
        mgram_edw2_url, 'edw2_meteogram.png', 'Edwards AFB Meteogram',
        nearest1hr_shortStr, nearest1hr_locStr, nearest1hr_delta
    ),
    'skewt_current_edw': (
        ('http://weather.uwyo.edu/upperair/images/' + current12hr_fullStr[:10]
         + '.72393.skewt.parc.gif'), 'edw_skewt.gif', 'Vandenberg AFB Sounding',
    ...
    'viirs_current': (
        'https://realearth.ssec.wisc.edu/api/image?products=G18-ABI-CONUS-BAND10.100,WFIGS-Perimeters.100,'
        'WFIGS-Current.100&width=830&height=500&client=RealEarth&basemap=bw&labels=line&center=39.926588421909436,'
        f'-114.38964843750001&zoom=5&time={nearest1hr_fullStr[:4]}-{nearest1hr_fullStr[4:6]}'
        f'-{nearest1hr_fullStr[6:8]}+{nearest1hr_fullStr[8:10]}:00:00',
        'viirs.png', 'VIIRS Fires + GOES WV', nearest1hr_shortStr, nearest1hr_locStr, nearest1hr_delta
    ),
    'anl_d1_surf_conus': (
        'https://www.wpc.ncep.noaa.gov/NationalForecastChart/staticmaps/noaad1.png',
        'surfanl_d1_conus.png', 'Day 1 Surface Forecast',
        day1_shortStr, day1_locStr, day1_delta),
    'anl_d2_surf_conus': (
        'https://www.wpc.ncep.noaa.gov/NationalForecastChart/staticmaps/noaad2.png',
        'surfanl_d2_conus.png', 'Day 2 Surface Forecast',
        day2_shortStr, day2_locStr, day2_delta),
    'anl_d3_surf_conus': (
        ...
        for day in modelProducts[product]['times'].keys():
            ...
            if outsideRange==0: # Only gather URL and file paths if current model has data out to the current day
                ...
                for hour in modelProducts[product]['times'][day]: # Loop through the different forecast times
                    ...
                    fcst_fullStr, fcst_shortStr, fcst_locStr, fcst_delta = dt2string(fcst_dt, present_time, utc_offset)
                    ...
                    for scope in sorted(modelProducts[product]['scope']):
                        ...
                        if 'xsect' in scope: # Model cross section
                            ...
                            if model=='gefs':
                                ...
                                remote_file = ['https://tropicaltidbits.com/analysis/models/eps/' +
                                               ...
                                               str(int(fhr_nearest12hr/6)+1) + '.png'][0]
                            elif model == 'gfs': # added for NURTURE
                                fhr_string = str(int(fhr_nearest6hr / 6) + 1)
                                remote_file = [
                                    'https://www.tropicaltidbits.com/analysis/models/gfs/'
                                    f'{nearest6hr_fullStr[:10]}/gfs_{product}_{scope}_{fhr_string}.png'
                                ]
                        ...
                        elif ('land' in scope) or ('ocean' in scope): # GMAO/GEOS graphics
                            remote_file = (
                                f'https://fluid.nccs.nasa.gov/wxmaps/static//plots/{product}/0/'
                                f'{scope}/nasa.gmao.WHyMSIE.chem2d.G5FPFC.{product}.0.{scope}.'
                                f'{past12hr_fullStr[:4]}-{past12hr_fullStr[4:6]}-'
                                f'{past12hr_fullStr[6:8]}T{past12hr_fullStr[8:10]}:00:00.'
                                f'PT{str(fhr_past12hr).zfill(3)}H.png'
                            )
                            local_file = f'{product_name}.png'
                            product_string = f'{model.upper()} {modelProducts[product]["name"]}'
                            
                            # query GMAO link since graphic not available until queried
                            query_gmao(product, scope, past12hr, fhr_past12hr)
                        else: # Traditional plan view forecast graphics
                            if (model=='gfs') or (model=='ecmwf') or (model=='nam') or (model=='nam3km') or (model=='hrrr') or (model=='rgem') or (model=='fv3'): # Tropical Tidbits graphics
                                if model=='gfs':
                                    if (product == 'ref_frzn') or (product == 'mslp_pcpn_frzn'):
                                    ...
                                    if model=='hrrr': # try later 12Z run
                                        hrr_latest_fullstr = (nearest6hr + timedelta(hours=6)).strftime('%Y%m%d%H%M')
                                        fhr_string = [
                                            str(int(fhr_nearest6hr)-6) if product=='ref3km_frzn' else
                                            str(int(fhr_nearest6hr)-5)
                                        ][0]
                                        if product=='ref3km_frzn':
                                            remote_file = (
                                                'https://tropicaltidbits.com/analysis/models/'
                                                + model + '/' + hrr_latest_fullstr[:10]
                                        ...
                                        if (product == 'ref3km_frzn'):
                                            fhr_string = str(int(fhr_nearest6hr))
                                        elif (model == 'nam3km') and (
                                                ('cfrac' in product) or (product == 'ir')):
                                            fhr_string = str(int(fhr_nearest6hr))
                                        else:
                                            fhr_string = str(int(fhr_nearest6hr) + 1)
...
#             prs, 'Past {} Hours'.format(str(-1*past12hr_delta)), -1, past12hr, present_time)
    ...
    if briefingUpdate=='True':
        prs.save(
            '{}/report.weather_{}.{}.discussion_new.pptx'.format(
                presentationPath, region, datetime.strftime(present_time, '%Y%m%d%H%M')
            )
        )
    else:
        prs.save(
            '{}/report.weather_{}.{}.discussion.pptx'.format(
                presentationPath, region, datetime.strftime(present_time, '%Y%m%d%H%M')
            )
        )
